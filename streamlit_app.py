...
with st.sidebar:
  ...
  flipper_length_mm = st.slider('Flipper Length (mm)', min(data['flipper_length_mm']), max(data['flipper_length_mm']), float(data['flipper_length_mm'].mean()))
  body_mass_kg = st.slider('Body Mass (kg)', min(data['body_mass_kg']), max(data['body_mass_kg']), float(data['body_mass_kg'].mean()))
